...
#define CRYPTO_LEN 2048
#define FIRST_SECTION 256

static z_stream stream;

static int uncompress(void *dst, void *src, int srclen)
{
	int err;
...
static int decypt_uim(unsigned long addr)
{
	void *ori, *src;
	void *dst = (void *) (addr + 64);
	int dst_len = CRYPTO_LEN;
	unsigned long src_len = *((unsigned long *) (addr + CRYPTO_LEN + 64 - 4));
	unsigned long flag = *((unsigned long *) (addr + CRYPTO_LEN + 64 - 8));

	if (flag != 0xacb0123) {
		printf("no encrypto mode\n");
		return 0;
	}

	src = malloc(CRYPTO_LEN);
	...
	ori = (void *) (addr + CRYPTO_LEN + 64 - 16 - src_len);
	memcpy(src + FIRST_SECTION, ori, src_len - FIRST_SECTION);

	ori = (void *) (addr + CRYPTO_LEN + 64 - 16 - FIRST_SECTION);
	memcpy(src, ori, FIRST_SECTION);

	dst_len = uncompress((void *) dst, (void *) src, src_len);
	if (dst_len < 0) {
		printf("de-crypto fail\n");
		return -1;
	}
