...
int fdt_initrd(void *fdt, ulong initrd_start, ulong initrd_end, int force)
{
	...
	if ((initrd_start == 0) || (initrd_end == 0)) {
		printf("WARNING: "
			"fdt_initrd initrd_start=%d,initrd_end=%d\n",
...
int fdt_fixup_memory_banks(void *blob, u64 start[], u64 size[], int banks)
{
	...
	/*if cfg memory size , use cfg, add by lty*/
	cell = fdt_getprop(blob, nodeoffset, "reg", NULL);
	if (cell != NULL && fdt32_to_cpu(cell[1]) != 0) {
		return 0;
	}
...
extern uint32_t org_fdt_chksum;
void fdt_get_checksum(int index)
{
	size_t i, fdt_size, fdt_total;
	uint32_t cur_chksum;
	u8 *p = (u8 *) gd->fdt_blob;

	fdt_total = fdt_totalsize(gd->fdt_blob);
	fdt_size = ALIGN(fdt_total, 32);
	for (i = fdt_total; i < fdt_size; i++)
		*(p + i) = 0;

	cur_chksum = crc32_wd(0, (const unsigned char *) gd->fdt_blob, fdt_size, CHUNKSZ_CRC32);
	printf("%d. checksum total(%zu), pad(%zu) Bytes FDT at: %08lx, crc: org(%08X) cur(%08X)\n",
		index, fdt_total, fdt_size, (ulong) gd->fdt_blob, org_fdt_chksum, cur_chksum);
}
