...
struct pinctrl_dev_info {
	...
	const struct owlxx_pinconf_pad_info *padinfo;
	const struct owlxx_pinctrl_pin_desc *pins;
	unsigned npins;
	const struct owlxx_pinctrl_func *functions;
	unsigned nfunctions;
	const struct owlxx_pinctrl_group *groups;
	unsigned ngroups;
};
		...
		const char *name)
{
	...
	const struct owlxx_pinctrl_pin_desc *pins = pinctrl->pins;
		...
		const char *name)
{
	...
	const struct owlxx_pinctrl_group *groups = pinctrl->groups;
		...
		const char *name)
{
	...
	const struct owlxx_pinctrl_func *functions = pinctrl->functions;
...
static inline int get_group_mfp_mask_val(
		const struct owlxx_pinctrl_group *g,
...
/*No more errors should happen, otherwise it will be hard to roll back*/
	...
	for (i = 0; i < g->padcnt; i++) {
		int pad_num;
		const struct owlxx_pinconf_pad_info *pad_info;
		struct owlxx_pinconf_schimtt *schimtt_info;
...
static int owlxx_pinctrl_group_pinconf_reg(
		const struct owlxx_pinctrl_group *g,
		enum owlxx_pinconf_param param,
		u32 *reg, u32 *bit, u32 *width)
{
	switch (param) {
	case OWLXX_PINCONF_PARAM_PADDRV:
...
static int owlxx_pinctrl_group_pinconf_arg2val(
		const struct owlxx_pinctrl_group *g,
		enum owlxx_pinconf_param param,
		u32 arg, u32 *val)
{
		...
		const char *group_name, unsigned long config)
{
	...
	u32 val = 0, mask = 0;
	u32 tmp;
	enum owlxx_pinconf_param param = OWLXX_PINCONF_UNPACK_PARAM(config);
	u32 arg = OWLXX_PINCONF_UNPACK_ARG(config);
	...
	g = &pinctrl->groups[group];
	ret = owlxx_pinctrl_group_pinconf_reg(g, param, &reg, &bit, &width);
	if (ret)
		return ret;

	ret = owlxx_pinctrl_group_pinconf_arg2val(g, param, arg, &val);
	if (ret)
		return ret;
...
static int owlxx_pad_pinconf_reg(
		const struct owlxx_pinconf_pad_info *pad,
		enum owlxx_pinconf_param param,
		u32 *reg, u32 *bit, u32 *width)
{
	switch (param) {
	case OWLXX_PINCONF_PARAM_PULL:
		...
		*width = pad->pull->width;
		break;
	case OWLXX_PINCONF_PARAM_SCHMITT:
...
static int owlxx_pad_pinconf_arg2val(
		const struct owlxx_pinconf_pad_info *pad,
		enum owlxx_pinconf_param param,
		u32 arg, u32 *val)
{
	...
	case OWLXX_PINCONF_PARAM_PULL:
		switch (arg) {
		case OWLXX_PINCONF_PULL_NONE:
			*val = 0;
			break;
		case OWLXX_PINCONF_PULL_DOWN:
			if (pad->pull->pulldown)
				*val = pad->pull->pulldown;
			else
				return -1;
			break;
		case OWLXX_PINCONF_PULL_UP:
			if (pad->pull->pullup)
				*val = pad->pull->pullup;
			else
				return -1;
...
{
	int ret = 0;
	const struct owlxx_pinconf_pad_info *pad_tab;
	u32 reg = 0, bit = 0, width = 0;
	u32 val = 0, mask = 0;
	u32 tmp;
	enum owlxx_pinconf_param param = OWLXX_PINCONF_UNPACK_PARAM(config);
	...
	ret = owlxx_pad_pinconf_reg(pad_tab, param, &reg, &bit, &width);
	if (ret)
		return ret;

	ret = owlxx_pad_pinconf_arg2val(pad_tab, param, arg, &val);
	if (ret)
		return ret;
...
static int pinctrl_dev_register(const char *name,
		int dev_node,
		const struct owlxx_pinconf_pad_info *padinfo,
		const const struct owlxx_pinctrl_pin_desc *pins,
		int npins,
		const struct owlxx_pinctrl_func *functions,
		int nfunctions,
		const struct owlxx_pinctrl_group *groups,
...
static const struct cfg_param cfg_params[] = {
	{"actions,pull",			OWLXX_PINCONF_PARAM_PULL},
	{"actions,paddrv",		OWLXX_PINCONF_PARAM_PADDRV},
/*	{"actions,schmitt",		OWLXX_PINCONF_PARAM_SCHMITT},*/
};
...
int owlxx_device_fdtdec_set_pinctrl_default(int dev_offset)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (state_node > 0) {
			ret = owlxx_fdtdec_set_pinctrl(state_node);
			if (ret)
				return ret;
		}
	}
...
int owlxx_pinctrl_set_function(
...
int owlxx_pinctrl_pin_set_configs(
		const char *pinctrl_dev_name,
		const char *pin_name,
...
int owlxx_pinctrl_group_set_configs(
		const char *pinctrl_dev_name,
		const char *group_name,
		unsigned long *configs,
		int num_configs)
...
int pinctrl_init_f(void)
{
	...
	dev_node = fdtdec_next_compatible(gd->fdt_blob,
		0, COMPAT_ACTIONS_OWLXX_PINCTRL);
	...
	if (hog_node > 0) {
		ret = owlxx_fdtdec_set_pinctrl(hog_node);
		if (ret)
			return ret;
	}
