...
int g_dnl_bind_fixup(struct usb_device_descriptor *dev, const char *name)
{
	...
	} else if (!strcmp(name, "usb_dnl_adfu")) {
		dev->idVendor = __constant_cpu_to_le16(CONFIG_G_DNL_ADFU_VENDOR_NUM);
		dev->idProduct = __constant_cpu_to_le16(CONFIG_G_DNL_ADFU_PRODUCT_NUM);
#endif
	} else {
