  ...
  switch (node->type)
  {
    case MXML_ELEMENT :
        if (node->value.element.name)
            mxml_mfree(node->value.element.name);
	...
	if (node->value.element.num_attrs)
	{
	  for (i = 0; i < node->value.element.num_attrs; i ++)
	  {
	    if (node->value.element.attrs[i].name)
	        mxml_mfree(node->value.element.attrs[i].name);
	    if (node->value.element.attrs[i].value)
	        mxml_mfree(node->value.element.attrs[i].value);
	  }

	  mxml_mfree(node->value.element.attrs);
	}
        break;
    case MXML_INTEGER :
       /* Nothing to do */
...
//        break;
    case MXML_TEXT :
        if (node->value.text.string)
            mxml_mfree(node->value.text.string);
        break;
    case MXML_CUSTOM :
...
#ifdef DEBUG
  printf("mxmlNewCDATA(parent=%p, data=\"%s\")\n",
...
#ifdef DEBUG
  printf("mxmlNewCustom(parent=%p, data=%p, destroy=%p)\n", parent,
          data, destroy);
#endif /* DEBUG */
...
#ifdef DEBUG
  printf("mxmlNewReal(parent=%p, real=%g)\n", parent, real);
#endif /* DEBUG */
