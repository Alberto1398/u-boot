...
int platform_get_lcd_par(struct owlxx_lcd *lcd_par)
{
	debug("platform lcd: pltfm get lcd par\n");

	lcd_par->port_type = LCD_PORT_TYPE_RGB;
	lcd_par->mode = &owlxx_mode_800_480;
...
int platform_get_dsi_par(struct owlxx_dsi *dsi_par)
{
	debug("platform dsi: pltfm get dsi par\n");

	dsi_par->mode = &owlxx_mode_800_480;
	return 0;
}
