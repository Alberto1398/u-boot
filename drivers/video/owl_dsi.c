/*
 * owlxx_dsi.c - OWL display driver
 *
...
static struct owlxx_dsi dsi_par;
...
struct data_fmt_param date_fmts[] = {
	{"rgb565", MIPI_RGB565},
	{"rgb666", MIPI_RGB666},
	{"rgb666lp", MIPI_RGB666_LP},
	{"rgb888", MIPI_RGB888},
};

u32 string_to_data_fmt(const char *name)
{
	int i;
...
int get_dsi_data_width(struct owlxx_dsi *par)
{
	switch (par->data_fmt) {
	case MIPI_RGB565:
		return 16;
...
static int dsi_activate_gpio(struct owlxx_fdt_gpio_state *gpio)
{
	int active_level;
...
static int dsi_deactivate_gpio(struct owlxx_fdt_gpio_state *gpio)
{
	int active_level;
...
static int gpio_bit_to_pwr(struct owlxx_fdt_gpio_state *gpio)
{
	int tmp = 0;
	if(gpio->name){
...
void gpioinit(void)
{
	...
	temp = readl(MFP_CTL1);
	temp &= 0xffe1c07f;	//
	writel(temp, MFP_CTL1);//[11:10]=b'11, select LCD_D9	[6:5]=b'11, select LCD_D14,D13
	
	temp = readl(MFP_CTL2);
	temp &= 0x98ffffff;	//
	writel(temp, MFP_CTL2);//	
	
	temp = readl(CMU_PWR_CTL);
	temp |= (0xf<<4);	//
	writel(temp, CMU_PWR_CTL);//	
	
	temp = readl(GPIO_DOUTEN);
	temp |= (1<<30);	//
	writel(temp, GPIO_DOUTEN);//
	
	temp = readl(GPIO_DDAT);
	//temp &= (~(1<<19));
	temp |= (1<<30);	
	writel(temp, GPIO_DDAT);//	
	mdelay(10);
...
{	
	...
	do{
	tmp=readl(DSI_PHY_CTRL);
	}while((tmp&0x02000000));//wait for cal done*/
...
int __platform_get_dsi_par(struct owlxx_dsi *par)
{
	return -1;
}

int platform_get_dsi_par(struct owlxx_dsi *)
...
static int fdtdec_enable_dsi(void)
{
	...
	printf("$$$$$$$$$$$$$$MFP_CTL1=0x%x ,MFP_CTL2=0x%x\n",readl(MFP_CTL1),readl(MFP_CTL2));

	ret = owlxx_device_fdtdec_set_pinctrl_default(
		dsi_par.dev_node);
	if (ret){
		printf("err:owlxx_device_fdtdec_set_pinctrl_default,ret=%d\n",ret);
	//	return -1;
	}
	printf("$$$$$$$$$$$$$$MFP_CTL1=0x%x ,MFP_CTL2=0x%x\n",readl(MFP_CTL1),readl(MFP_CTL2));
	enable_dsi();
	...
	pd->power = 1;
	owlxx_pwm_backlight_update_status(pd);
...
int fdtdec_get_dsi_par(struct owlxx_dsi *par)
{
	int dev_node;
	int mode_node;
	int len;
	...
	dev_node = fdtdec_next_compatible(
		gd->fdt_blob, 0, COMPAT_ACTIONS_OWLXX_DSI);
	if (dev_node <= 0) {
	...
	mode_node = fdtdec_lookup_phandle(
			gd->fdt_blob, dev_node, "videomode");
	if (mode_node <= 0) {
	...
	data_fmt_name = fdt_getprop(
			gd->fdt_blob, dev_node, "data_format", &len);
	debug("fdtdec_get_dsi_par 1\n");

	par->data_fmt = string_to_data_fmt(data_fmt_name);
	debug("fdtdec_get_dsi_par 2\n");

	par->data_lane_num = fdtdec_get_int(
			gd->fdt_blob, dev_node, "data_lane_num", -1);
	debug("fdtdec_get_dsi_par 3\n");
...
int owlxx_dsi_init(void)
{
	...
	if (owlxx_pwm_backlight_init(&dsi_par.pwm_bl_data)) {
		printf("OWL dsi: backlight not found\n");
		return -1;
	}
	data_width = get_dsi_data_width(&dsi_par);

	owlxx_display_register(DSI_DISPLAYER, &dsi_ops, dsi_par.mode,data_width,0);
