 ...
 */
#define DEBUGX
#define pr_fmt(fmt) "owl_de_s900: " fmt

#include <asm/io.h>

#include <asm/arch/regs_s900.h>		/* TODO */
...
static void de_s900_video_apply_info(struct owl_de_video *video)
{
	...
	debug("%s: global_alpha %d, global_alpha_en %d, pre_mult_en %d\n",
	      __func__, info->global_alpha, info->global_alpha_en,
	      info->pre_mult_alpha_en);
	__video_alpha_set(video->path, video, 0xff, true, false);
}
...
static int de_s900_device_power_on(struct owl_de_device *de)
{
	...
	/* wait power on ack */
	i = 1000;
	while (i-- && ((readl(SPS_PG_ACK) & (1 << 11)) == 0))
		mdelay(1); /* bit 13 is DS ACK */

	debug("%s: wait res %d, %x\n", __func__, i, readl(SPS_PG_ACK));
