...
s32 hdmi_set_tvoutpll_by_pixclk(int vid, int deep_color, int _3d)
{
    ...
    switch (vid) {      
    case OWLXX_TV_MOD_576P:
    case OWLXX_TV_MOD_480P :  
        if (_3d == _3D) { 
        ...
        break;
    case OWLXX_TV_MOD_720P_60HZ: 
    case OWLXX_TV_MOD_720P_50HZ:
        if (_3d == _3D) { 
    ...
    case OWLXX_TV_MOD_1080P_60HZ://need check
    case OWLXX_TV_MOD_1080P_50HZ:
        if (_3d == _3D) { 
...
int fdtdec_get_hdmi_par(struct hdmi_sink_info* psink_info)
{
    int dev_node;
    ...
    debug("%s\n", __func__);
/*
    dev_node = fdtdec_next_compatible(gd->fdt_blob, 0, COMPAT_ACTIONS_OWLXX_HDMI);
    if (dev_node <= 0) {
        printf("%s: Can't get gl5203-hdmi device node\n", __func__);
        return -1;
    }

    resolution = fdt_getprop(gd->fdt_blob, dev_node, "resolution", &len);
    debug("%s: resolution  = %s\n",__func__, resolution);
    printf("%s: resolution  = %s, len=%d\n",__func__, resolution, len);

    vid = string_to_data_fmt(resolution);
    if (valide_vid(vid)){
        psink_info->v_settings.vid = vid;
    } else {
        printf("%s: error vid  = %d\n",__func__, vid);
        ret = -2;
    }
*/
	psink_info->v_settings.vid = OWLXX_TV_MOD_720P_60HZ; 
    return ret;
}


#ifdef CONFIG_CMD_EXT4
#define EXT4_DATA_DEVICE 5
...
int hdmi_init(void)
{
    ...
    if (!read_usr_cfg_file(HDMI_SETTING_MODE_PATH, buf)){
        ...
        printf("%s: Get mode from dts\n", __func__);
        if (fdtdec_get_hdmi_par(psink_info)) {
    ...
    owlxx_display_register(HDMI_DISPLAYER, &hdmi_ops, &(v_mode->mode),24,3);
