...
static struct owlxx_lcd lcd_par;
...
int __platform_get_lcd_par(struct owlxx_lcd *par)
{
	return -1;
}

int platform_get_lcd_par(struct owlxx_lcd *)
...
static int lcd_activate_gpio(struct owlxx_fdt_gpio_state *gpio)
{
	int active_level;
...
static int lcd_deactivate_gpio(struct owlxx_fdt_gpio_state *gpio)
{
	int active_level;
...
static int fdtdec_enable_lcd(void)
{
	...
	owlxx_regulator_enable(lcd_par.lcdvcc_regulator);
...
static int fdtdec_disable_lcd(void)
{
	...
	owlxx_regulator_disable(lcd_par.lcdvcc_regulator);
...
int fdtdec_get_lcd_par(struct owlxx_lcd *par)
{
	int dev_node;
	int mode_node;
	...
	dev_node = fdtdec_next_compatible(
		gd->fdt_blob, 0, COMPAT_ACTIONS_OWLXX_LCD);
	if (dev_node <= 0) {
	...
	par->lcdvcc_regulator = fdtdec_owlxx_regulator_get(gd->fdt_blob, dev_node, "lcdvcc-supply");

	return 0;
}

int owlxx_lcd_init(void)
{
	int lcdic_type;
	...
	if (owlxx_pwm_backlight_init(&lcd_par.pwm_bl_data)) {
		printf("OWL LCD: backlight not found\n");
		return -1;
	}

	owlxx_display_register(LCD_DISPLAYER, &lcd_ops, lcd_par.mode,lcd_par.data_width,0);
	return 0;
}
