...
static void _atc260x_rtc_unpack_reg(struct atc260x_rtc_time *rtc_tm,
				    unsigned int raw_rtc_ms, unsigned int raw_rtc_h,
				    unsigned int raw_rtc_ymd, unsigned int raw_rtc_cen)
{
...
int atc260x_rtc_tm_to_time(unsigned long *time)
{
	...
	if (ret)
		return ret;
	printf("%d %d %d %d %d %d", rtc_tm.year, rtc_tm.mon, rtc_tm.dom, rtc_tm.hour, rtc_tm.min, rtc_tm.sec);
	*time = _mktime(rtc_tm.year, rtc_tm.mon, rtc_tm.dom,
			rtc_tm.hour, rtc_tm.min, rtc_tm.sec);
...
int atc260x_time_to_rtc_tm(unsigned long time)
{
	...
	printf("%d %d %d %d %d %d\n", rtc_tm.year, rtc_tm.mon, rtc_tm.dom, rtc_tm.hour, rtc_tm.min, rtc_tm.sec);
