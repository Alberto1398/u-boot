...
/**
 * The connect_to_pc flag was set in reset interrupt, otherwise some
 * usb adapter are so weird that might generate signal just like reset
 * signal from PC.
 * Move the flag here to avoid that problem.
 * connect done interrupt is just following the reset interrrupt.
 */
#ifdef CONFIG_USB_CONNECT_DETECT
	connect_to_pc = 1;
#endif
