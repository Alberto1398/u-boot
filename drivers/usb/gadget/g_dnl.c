...
static int g_dnl_do_config(struct usb_configuration *c)
{
	...
	if (!strcmp(s, "usb_dnl_dfu"))
		...
		device_desc.idVendor = __constant_cpu_to_le16(CONFIG_G_DNL_UMS_VENDOR_NUM);
		ret = fsg_add(c);
        }
	else if (!strcmp(s, "usb_dnl_fboot")){
		device_desc.idVendor = __constant_cpu_to_le16(CONFIG_G_DNL_VENDOR_NUM);
		ret = fastboot_add(c);
        }
...
int g_dnl_register(const char *type)
{
	...
	static char name[sizeof(shortname) + 6];
	int ret;
