...
int owlxx_gpio_generic_direction_input(struct gpio_chip *chip, unsigned gpio)
{
	if (chip)
...
int owlxx_gpio_generic_direction_output(struct gpio_chip *chip, unsigned gpio, int value)
{
	if (chip)
...
int owlxx_gpio_generic_get_value(struct gpio_chip *chip, unsigned gpio)
{
	if (chip)
...
int owlxx_gpio_generic_set_value(struct gpio_chip *chip, unsigned gpio, int value)
{
	if (chip)
 ...
 */
int owlxx_fdtdec_decode_gpios(const void *blob, int node,
...
int owlxx_fdtdec_decode_gpio(const void *blob, int node,
		const char *prop_name,
		struct owlxx_fdt_gpio_state *gpio)
{
	...
	gpio->name = NULL;
	err = owlxx_fdtdec_decode_gpios(blob, node, prop_name, gpio, 1);
	return err == 1 ? 0 : err;
}
