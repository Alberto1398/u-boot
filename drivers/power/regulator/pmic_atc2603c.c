...
static int dcdc_reg_ctl[3] = { ATC2603C_PMU_DC0_CTL0,
	ATC2603C_PMU_DC1_CTL0,
	ATC2603C_PMU_DC2_CTL0,
	ATC2603C_PMU_DC3_CTL0
};

static int ldo_reg_ctl[9] = { ATC2603C_PMU_LDO0_CTL,
...
static int atc2603c_ldo_get_value(struct udevice *dev)
{
	...
	case 11:
		step = buf[0] >> 5;
		printf("ldo%d %d uV", 100000 * step + 2600000);
		break;
	case 3:
	case 7:
		step = buf[0] >> 5;
		printf("ldo%d %d uV", 100000 * step + 1500000);
		break;
	case 6:
		step = buf[0] >> 3;
		printf("ldo%d %d uV", 25000 * step + 700000);
		break;
	case 8:
		step = buf[0] >> 4;
		printf("ldo%d %d uV", 100000 * step + 2300000);
		break;
	}
	return uV;
}
...
static int atc2603c_ldo_set_value(struct udevice *dev, int uV)
{
	...
	case 11:
		if ((uV < 2600000) || (uV > 3300000)) {
			...
			if (ret)
				return ret;
			printf("Set ldo%d %d uV", 100000 * step + 2600000);
		}
		break;
	case 3:
	case 7:
		if ((uV < 1500000) || (uV > 2000000)) {
			...
			if (ret)
				return ret;
			printf("Set ldo%d %d uV", 100000 * step + 2000000);
		}
		break;
	case 6:
		if ((uV < 700000) || (uV > 1400000)) {
			...
			if (ret)
				return ret;
			printf("Set ldo%d %d uV", 25000 * step + 700000);
		}
		break;
	case 8:
		if ((uV < 2300000) || (uV > 3300000)) {
			...
			if (ret)
				return ret;
			printf("Set ldo%d %d uV", 100000 * step + 2300000);
		}
		break;
	}
}
...
U_BOOT_DRIVER(atc2603c_ldo) = {
		.name = ATC2603C_LDO_DRIVER,
		.id = UCLASS_REGULATOR,
		.ops = &atc2603c_ldo_ops,
		.probe = atc2603c_ldo_probe,
};
...
U_BOOT_DRIVER(atc2603c_dcdc) = {
		.name = ATC2603C_DCDC_DRIVER,
		.id = UCLASS_REGULATOR,
		.ops = &atc2603c_dcdc_ops,
		.probe = atc2603c_dcdc_probe,
};
