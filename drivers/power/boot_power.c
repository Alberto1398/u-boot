...
/* PMU_CHARGER_CTL1 */
static const u16 pmu_charger_ctl1[OWLXX_PMU_ID_CNT] = {
	[OWLXX_PMU_ID_ATC2603A] = ATC2603A_PMU_CHARGER_CTL1,
	[OWLXX_PMU_ID_ATC2603B] = -1,//ATC2609A_PMU_CHARGER_CTL1,
	[OWLXX_PMU_ID_ATC2603C] = ATC2603C_PMU_CHARGER_CTL1
};
/* PMU_BATVADC */
static const u16 pmu_batvadc[OWLXX_PMU_ID_CNT] = {
	[OWLXX_PMU_ID_ATC2603A] = ATC2603A_PMU_BATVADC,
	[OWLXX_PMU_ID_ATC2603B] = ATC2609A_PMU_BATVADC,
	[OWLXX_PMU_ID_ATC2603C] = ATC2603C_PMU_BATVADC
};

/* PMU_WALLVADC */
static const u16 pmu_wallvadc[OWLXX_PMU_ID_CNT] = {
	[OWLXX_PMU_ID_ATC2603A] = ATC2603A_PMU_WALLVADC,
	[OWLXX_PMU_ID_ATC2603B] = ATC2609A_PMU_WALLVADC,
	[OWLXX_PMU_ID_ATC2603C] = ATC2603C_PMU_WALLVADC
};
/* PMU_VBUSVADC */
static const u16 pmu_vbusvadc[OWLXX_PMU_ID_CNT] = {
	[OWLXX_PMU_ID_ATC2603A] = ATC2603A_PMU_VBUSVADC,
	[OWLXX_PMU_ID_ATC2603B] = ATC2609A_PMU_VBUSVADC,
	[OWLXX_PMU_ID_ATC2603C] = ATC2603C_PMU_VBUSVADC
};
/* PMU_SYS_CTL3 */
static const u16 pmu_sys_ctl3[OWLXX_PMU_ID_CNT] = {
...
/* PMU_SYS_CTL1 */
static const u16 pmu_sys_ctl1[OWLXX_PMU_ID_CNT] = {
...
/* PMU_SYS_CTL5 */
static const u16 pmu_sys_ctl5[OWLXX_PMU_ID_CNT] = {
	[OWLXX_PMU_ID_ATC2603A] = ATC2603A_PMU_SYS_CTL5,
	[OWLXX_PMU_ID_ATC2603B] = ATC2609A_PMU_SYS_CTL5,
	[OWLXX_PMU_ID_ATC2603C] = ATC2603C_PMU_SYS_CTL5
};
/* PMU_SYS_CTL0 */
static const u16 pmu_sys_ctl0[OWLXX_PMU_ID_CNT] = {
...
/* PMU_SYS_CTL9 */
static const u16 pmu_sys_ctl9[OWLXX_PMU_ID_CNT] = {
	[OWLXX_PMU_ID_ATC2603A] = ATC2603A_PMU_SYS_CTL9,
	[OWLXX_PMU_ID_ATC2603B] = ATC2609A_PMU_SYS_CTL9,
	[OWLXX_PMU_ID_ATC2603C] = ATC2603C_PMU_SYS_CTL9
};
...
int is_battery_exist(void)
{
	...
	ret = atc260x_set_bits(pmu_charger_ctl1[OWLXX_PMU_ID],
	...
	ret = atc260x_reg_read(pmu_charger_ctl1[OWLXX_PMU_ID]);//depend on pmu.c
	if (ret < 0)
		return ret;
...
int get_bat_voltage(int *volt)
{
	...
	ret = atc260x_reg_read(pmu_batvadc[OWLXX_PMU_ID]);
	if (ret < 0)
		return ret;
...
void get_bat_capacity(int *cap,int bat_mv)
{
	...
	for (i=0; i < 11; i++) 
	{
		if (bat_mv < table_bat_percentage[i][0]) 
		{
	...
	pmu_sys_ctl9_data = atc260x_reg_read(pmu_sys_ctl9[OWLXX_PMU_ID]);
...
int get_charge_plugin_status(int* wall_mv, int* vbus_mv)
{
	...
	ret = atc260x_reg_read(pmu_wallvadc[OWLXX_PMU_ID]);
	if(ret >= 0)
	{
		ret *= 2930;		/* 2.93mv * 2.5 */
		ret = ret * 2 + ret / 2;
	...
	ret = atc260x_reg_read(pmu_vbusvadc[OWLXX_PMU_ID]);
	if(ret >= 0)
	{
		ret *= 2930;            /* 2.93mv * 2.5 */
		ret = ret * 2 + ret / 2;
...
int get_adaptor_type(char *type)
{
	const void *blob = gd->fdt_blob;
	int node;
	node = fdt_path_offset(blob, "/spi@b0204000");
	type = fdt_getprop(blob, node, "support_adaptor_type", NULL);
	if(type)
	{
		printf("dts:support_adaptor_type= %s\n", type);
		return 1;
	}
	else
	{
		printf("Cannot find any supporting adapter !");
		return 0;
	}
 ...
 {
	atc260x_set_bits(
		pmu_sys_ctl3[OWLXX_PMU_ID],
		PMU_SYS_CTL3_EN_S2 | PMU_SYS_CTL3_EN_S3,
		0);
	atc260x_set_bits(pmu_sys_ctl1[OWLXX_PMU_ID], PMU_SYS_CTL1_EN_S1, 0);
	return;
}
static void buck_pwm_config()
{
	/* set pwm div,bit12=1: 24M */
    writel(0x1000, CMU_PWM3CLK);
    /*
	 * bit0~bit9=63:DUTY_ALL,bit20=0:POL_SEL
	 * duty cycle=1-DUTY_ACTIVE/DUTY_ALL=1
	 */      
    writel(readl(PWM_CTL3) | 63, PWM_CTL3);   
	 /*enable pwm3 clk*/
	writel(readl(CMU_DEVCLKEN1) | (0x1<<26), CMU_DEVCLKEN1);
	/* DUTY_ACTIVE = 63 */
	writel(readl(PWM_CTL3) | (0x3f<<10), PWM_CTL3); 

}
void check_power(void)
{
	char support_adaptor_type[3];
	...
	{
		if(!strcmp(support_adaptor_type,"1"))
		{
			atc260x_set_bits(pmu_sys_ctl5[OWLXX_PMU_ID], (1<<8), 0); /* disable VBUS wake */	
			mdelay(10);
			atc260x_set_bits(pmu_sys_ctl0[OWLXX_PMU_ID], (1<<15), 0); 
			mdelay(10);			
		}
	}
	...
	if(charge_plugin_status != NO_PLUG)
	{
		 ...
		 */
		if(bat_mv < 3300 && charge_plugin_status == USB_PLUG)
		{
