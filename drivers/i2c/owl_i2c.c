/*
 * arch/arm/mach-leopard/owlxx_i2c.c
 *
...
static inline void owlxx_i2c_mfp_config(u32 bus)
{
	int  ret;
	switch (bus) {
	case 0:
...
static inline void owlxx_i2c_pullup_enable(u32 bus)
{
	switch (bus) {
	case 0:
...
static inline void owlxx_i2c_pullup_disable(u32 bus)
{
	switch (bus) {
	case 0:
		/* PAD_PULLCTL0  bit 9:8 = 11
...
static inline void owlxx_i2c_clk_enable(u32 bus)
{
	uint reg_val;
...
static inline void owlxx_i2c_clk_disable(u32 bus)
{
	switch (bus) {
	case 0:
...
static struct owlxx_i2c *get_i2c_base(void)
{
	return (struct owlxx_i2c *)(I2C_BASE +
...
int i2c_set_bus_num(unsigned int bus)
{
	...
	owlxx_i2c_mfp_config(bus);
	owlxx_i2c_clk_enable(bus);
	owlxx_i2c_pullup_enable(bus);
 ...
 */
int i2c_transfer(struct owlxx_i2c *i2c,
...
void owlxx_dump_i2c_register(void)
{
	struct owlxx_i2c *i2c;
	u32 addr = 0;
	int i = 0;
...
struct owlxx_i2c_bus {
	int node;	/* device tree node */
	int bus_num;	/* i2c bus number */
};

static int i2c_busses __attribute__((section(".data")));
static struct owlxx_i2c_bus i2c_bus[CONFIG_MAX_I2C_NUM]
...
void board_i2c_init(const void *blob)
{
	...
	count = fdtdec_find_aliases_for_id(blob, "i2c",
		COMPAT_ACTIONS_OWLXX_I2C, node_list,
		CONFIG_MAX_I2C_NUM);

	for (i = 0; i < count; i++) {
		struct owlxx_i2c_bus *bus;
...
static struct owlxx_i2c_bus *get_bus(unsigned int bus_idx)
{
