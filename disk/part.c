...
#endif
#if defined(CONFIG_ACTS_STORAGE_NAND)	/* add by Actions */
	{ .name = "nandblk", .get_dev = act_nandblk_get_dev, },
#endif
	{ },
};
...
/* must be placed before DOS partition detection */
...
#ifdef CONFIG_ACTS_AFI_PARTITION	/* add by Actions */ /* MUST BE LAST! */
	if (test_part_actafi(dev_desc) == 0) {
	    dev_desc->part_type = PART_TYPE_ACTAFI;
	    return;
	}
#endif
	...
	defined(CONFIG_ACTS_AFI_PARTITION) || \
...
#ifdef CONFIG_ACTS_AFI_PARTITION	/* add by Actions */
	case PART_TYPE_ACTAFI:
	    PRINTF ("## Testing for a valid act_afi partition ##\n");
	    print_part_header ("ACTAFI", dev_desc);
	    print_part_actafi (dev_desc);
	    return;
#endif
...
#ifdef CONFIG_ACTS_AFI_PARTITION	/* add by Actions */
	case PART_TYPE_ACTAFI:
		if (get_partition_info_actafi(dev_desc, part, info) == 0) {
			PRINTF("## Valid act_afi partition found ##\n");
			return 0;
		}
		break;
#endif
